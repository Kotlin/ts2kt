...
@native
@nativeGetter
fun <T> Map<T>.get(index: String): T = noImpl
@native
@nativeSetter
fun <T> Map<T>.set(index: String, value: T): Unit = noImpl
@native
interface FileMap<T> {
    fun get(fileName: Path): T
    fun set(fileName: Path, value: T)
    fun contains(fileName: Path): Boolean
    fun remove(fileName: Path)
    fun forEachValue(f: (key: Path, v: T) -> Unit)
    fun clear()
}
@native
var TextRange.pos: Number = noImpl
@native
var TextRange.end: Number = noImpl
...
@native
var Node.kind: SyntaxKind = noImpl
@native
var Node.flags: NodeFlags = noImpl
@native
var Node.decorators: NodeArray<Decorator>? = noImpl
@native
var Node.modifiers: ModifiersArray? = noImpl
@native
var Node.parent: Node? = noImpl
@native
...
interface TypeChecker {
    ...
    fun getJsxElementAttributesType(elementNode: JsxOpeningLikeElement): Type
...
interface System {
    ...
    val watchFile: ((path: Path, callback: FileWatcherCallback) -> FileWatcher)? = noImpl
    val watchDirectory: ((path: String, callback: DirectoryWatcherCallback, recursive: Boolean? = null) -> FileWatcher)? = noImpl
...
interface Scanner {
    ...
    fun lookAhead<T>(callback: () -> T): T
    fun scanRange<T>(start: Number, length: Number, callback: () -> T): T
    fun tryScan<T>(callback: () -> T): T
}
@native
...
interface TranspileOptions {
    ...
    var renamedDependencies: Map<String>? = noImpl
}
@native
